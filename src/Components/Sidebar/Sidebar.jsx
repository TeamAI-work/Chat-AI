...
import { toggleTheme as toggleAppTheme } from "../../config/theme";
    ...
    <motion.div className="text-gray-900 dark:text-white flex flex-col h-full select-none">
      ...
      <div
        ...
        <div className="flex items-center gap-1">
          <button onClick={toggleTheme} className="p-1.5 rounded-xl text-theme-muted hover:text-theme-text hover:bg-gray-200 dark:hover:bg-white/10 transition-all">
      ...
      <div className="px-4 space-y-2 mb-8">
        <button onClick={onNewChat} className="flex items-center gap-3 bg-gradient-to-r from-theme-primary to-theme-secondary text-white w-full py-2.5 px-4 rounded-xl text-sm font-medium shadow-lg shadow-theme-primary/20 border border-theme-primary/30">
          <Plus size={18} strokeWidth={2.5} /> New chat
        </button>
        <button onClick={onNewProject} className="flex items-center gap-3 bg-gray-100 hover:bg-gray-200 border border-theme-border text-gray-700 dark:bg-white/5 dark:hover:bg-white/10 w-full py-2.5 px-4 rounded-xl text-sm font-medium dark:text-gray-300">
          <Plus size={18} strokeWidth={2.5} /> New Project
        </button>
        <div className="flex items-center gap-3 bg-gray-100 border border-theme-border text-gray-700 dark:bg-white/5 w-full px-4 rounded-xl text-sm dark:text-gray-300 h-[42px]">
              ...
              {filteredProjects.map(project => (
                <motion.div
                  ...
                  {expandedProjects.has(project.id) && (
                    <div className="ml-5 pl-3 border-l border-gray-200 dark:border-theme-border py-1">
      ...
      {/* User Profile */}
      <div className="p-4 mt-auto border-t border-gray-200 dark:border-theme-border relative">
        <div onClick={() => setProfileModel(!profileModel)} className="flex items-center gap-3 w-full hover:bg-gray-200 dark:hover:bg-white/5 p-2 rounded-xl transition-all cursor-pointer">
          <div className="w-9 h-9 bg-linear-to-tr from-theme-primary to-theme-secondary rounded-full flex items-center justify-center text-white font-bold">
        ...
        <AnimatePresence>
          {profileModel && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 10 }}
              className="absolute bottom-full left-0 mb-4 w-60 bg-white dark:bg-theme-surface border border-gray-200 dark:border-theme-border rounded-xl shadow-2xl p-2 z-50 ml-4"
            >
              <button
