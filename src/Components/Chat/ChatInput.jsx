  ...
  return (
    <div className="max-w-3xl mx-auto w-full">
      <motion.div
        ...
        className="bg-white/80 dark:bg-theme-surface/80 backdrop-blur-xl border border-theme-border rounded-full px-2 py-1 shadow-2xl focus-within:ring-1 focus-within:ring-theme-primary/50 transition-all">
        <div className="relative flex items-center justify-between gap-2">
          <button
            onClick={() => setUploadModel(!uploadModel)}
            className="p-3 text-theme-muted hover:text-theme-text hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-colors shrink-0"
          >
            <Plus size={20} />
          </button>
          ...
          <textarea
            ...
            className="w-full bg-transparent text-theme-text placeholder-gray-500 resize-none outline-none overflow-y-auto max-h-40 min-h-[44px] py-2.5 text-[15px] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 hover:[&::-webkit-scrollbar-thumb]:bg-gray-400 dark:hover:[&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full"
            value={value}
          ...
          <button
            ...
            ${isEmpty || disabled
                ...
                : "text-white bg-linear-to-tr from-theme-primary to-theme-secondary hover:brightness-110 active:scale-95"
              }`}
          >
            <Send size={20} />
          </button>
          ...
          <div
            ref={uploadRef}
            className={`absolute ${uploadModel ? "" : "hidden"} -top-52 px-3 py-4 left-0 bg-white dark:bg-theme-surface border border-theme-border backdrop-blur-xl rounded-2xl p-2 shadow-2xl flex flex-col gap-2 z-50 transition-all text-gray-800 dark:text-gray-200`}
          >
            <div>
        ...
        {(thinking || deepThinking || webSearch) && (
          <div className="flex justify-start gap-2 shrink mt-2 px-2 pb-1">
            {thinking && (
              <div className="flex items-center gap-2 hover:bg-gray-200 dark:hover:bg-white/10 px-2 py-1 rounded-full bg-theme-primary/30 cursor-pointer text-xs dark:text-gray-100">
